...
class PitchCountView extends Component {
    ...
    handleSelectPitcher(props) { // THIS METHOD IS CALLED AFTER SELECTING TEAM AND PLAYER IS COMPLETE IN PITCHER DISPLAY
        this.setState({ currentPitcher: props.currentPlayer, currentTeamID: props.currentTeamID, currentTeamName: props.currentTeamName })
    ...
    addNewPitch(e) {
        e.preventDefault()
        console.log(this.state.currentPitcher.playerFirstName)
        ...
        var newPitch = {
            ...
            "gameDate": new Date().toDateString(), // TODO: FORMAT THIS DATE NICE
        ...
        this.setState({ pitchesThisInning: statePitchArray, pitchInQueue: newPitch })
    }
    ...
    // 3. 
        ...
        var inning = this.state.currentInning + .5
        ...
        index.addObjects( newPitchArray, function() {
            this.setState( { pitchesThisInning: [], currentInning: inning, pitchInQueue: {}, viewState:NEW_INNING_VIEW })
...
countTotal() {
    if (this.state.pitchInQueue.gameDate) return this.state.pitchesThisInning.length + 1
    else return 0

}

countStrikes() {
    if (this.state.pitchInQueue.pitchResult === 'strike') return _.filter(this.state.pitchesThisInning, { pitchResult: 'strike' }).length + 1
    else return _.filter(this.state.pitchesThisInning, { pitchResult: 'strike' }).length


}

countBalls() {
    if (this.state.pitchInQueue.pitchResult === 'ball') return _.filter(this.state.pitchesThisInning, { pitchResult: 'ball' }).length + 1
    else return _.filter(this.state.pitchesThisInning, { pitchResult: 'ball' }).length

}

render() {
    // TODO...
    // * RENDER BATTER DISPLAY
    // * STYLE PITCHER AND BATTER DISPLAY WITH
    // ** BALLS, STRIKES, TOTAL
    return (
        <div className="grid-container">
            ...
            <PitchCounter addNewPitch={this.addNewPitch} endInning={this.endInning} />
